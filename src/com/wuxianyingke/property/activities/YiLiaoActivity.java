	...
	public void onPoiItemDetailSearched(PoiItemDetail result, int rCode) {
		...
		if (rCode == 0) {
			if (result != null) {// 搜索poi的结果
				...
				Toast.makeText(YiLiaoActivity.this, "没有信息", Toast.LENGTH_LONG).show();
			}
		} else if (rCode == 27) {

			Toast.makeText(YiLiaoActivity.this, "网络错误", Toast.LENGTH_LONG).show();
		} else if (rCode == 32) {
			Toast.makeText(YiLiaoActivity.this, "key错误", Toast.LENGTH_LONG).show();
		} else {
			Toast.makeText(YiLiaoActivity.this, "其他错误："+rCode, Toast.LENGTH_LONG).show();
		}
	}
			...
			if (result != null && result.getQuery() != null) {// 搜索poi的结果
				if (result.getQuery().equals(query)) {// 是否是同一条
					...
					if (poiItems != null && poiItems.size() > 0) {
						...
						Toast.makeText(YiLiaoActivity.this, "搜索完成", Toast.LENGTH_LONG).show();
					}  else {
						Toast.makeText(YiLiaoActivity.this, "没有任何结果", Toast.LENGTH_LONG).show();
					}
				}
			} else {
				Toast.makeText(YiLiaoActivity.this, "3000m范围类没有任何结果", Toast.LENGTH_LONG).show();
			}
		} else if (rCode == 27) {
