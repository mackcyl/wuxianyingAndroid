...
public class PropertyNotificationAdapter extends BaseAdapter {
	private List<PropertyNotificationInfo> mList;
	private Context mContext;
	private boolean mStoped;
	private int mCount;
	
	public PropertyNotificationAdapter(Context ctx, List<PropertyNotificationInfo> list) 
	{
		this.mContext = ctx;
		this.mList = list;
		this.mStoped = false;
	...
	public void appandAdapter(List<PropertyNotificationInfo> list) 
	{
	...
	public View getView(int position, View convertView, ViewGroup parent) 
	{
		...
		final PropertyNotificationInfo activity = mList.get(position);
		if (convertView == null) 
		{
		...
		activityItem.mProductMessageTimeTextView.setText(activity.dateTime);
		activityItem.mMainRadio4ListItemLinearLayout.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				intent.putExtra("productMessageInfoTime", activity.dateTime);
				intent.putExtra("productMessageInfoContent", activity.body);
				intent.putExtra("productMessageInfoSignature", activity.signature);
